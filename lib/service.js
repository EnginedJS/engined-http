...
module.exports = (opts = {}) => class extends Service {
	...
	async _setupMiddleware() {
		...
		// Setup view renderer
		let viewSettings = await this.setupViews();
		if (viewSettings !== null) {
			consolidate.requires[viewSettings.options.map.html] = viewSettings.templateEngine;
			this.agent.use(views(viewSettings.path, viewSettings.options));
		}

		// Customized middlewares
		this.agent.use(this._middlewares.bind(this));

		let staticServeSettings = await this.setupStaticServe();
		if (staticServeSettings !== null) {
			this.agent.use(serve(staticServeSettings.path, staticServeSettings.options));
		}
	}
